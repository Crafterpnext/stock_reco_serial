...
frappe.ui.form.on("Serialized Stock Reconciliation", {
	onload: function(frm) {
	...
	get_items: function(frm) {
		frappe.prompt({label:"Warehouse", fieldname: "warehouse", fieldtype:"Link", options:"Warehouse", reqd: 1,
			...
			function(data) {
				frappe.call({
					method:"stock_reco_serial.stock_reco_serial.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		if(d.item_code && d.warehouse) {
			frappe.call({
				method: "stock_reco_serial.stock_reco_serial.doctype.serialized_stock_reconciliation.serialized_stock_reconciliation.get_stock_balance_for",
				args: {
	...
	set_expense_account: function(frm) {
		if (frm.doc.company && erpnext.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "stock_reco_serial.stock_reco_serial.doctype.serialized_stock_reconciliation.serialized_stock_reconciliation.get_difference_account",
				args: {
...
frappe.ui.form.on("Serialized Stock Reconciliation Item", {
